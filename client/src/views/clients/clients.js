...
import { getClients, saveClient, getOrders } from '../../api/index.js';
...
export function renderClients(q) {
  const clients = getClients(); const orders = getOrders();
...
export function openClientModal(cid) {
  const c = getClients().find(x => x.id === cid); if (!c) return;
  const ords = getOrders().filter(o => o.clientId === cid).sort((a, b) => b.createdAt.localeCompare(a.createdAt));
...
export function saveNewClient() {
  ...
  const c = { id:'c'+Date.now(), name, company:document.getElementById('nc_company').value||'', phone, email:document.getElementById('nc_email').value||'', address:addr, createdAt:new Date().toISOString().split('T')[0] };
  saveClient(c); closeModal(); showToast(`Client ${name} added`, 'toast-gold'); renderClients();
}
