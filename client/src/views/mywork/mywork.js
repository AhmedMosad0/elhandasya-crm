...
export async function renderMyWork() {
  ...
  try {
    ...
    const active = mine.filter(o => o.status === 'pending' || o.status === 'mixing');
    ...
    ${ready.length ? `<div style="margin:16px 0 8px;font-size:11.5px;font-weight:700;color:var(--accent);text-transform:uppercase;letter-spacing:.5px">Ready for Delivery (${ready.length})</div>${ready.map(o => _workCard(o, role)).join('')}` : ''}
