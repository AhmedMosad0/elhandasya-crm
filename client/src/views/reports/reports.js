...
export async function renderReports() {
  document.getElementById('sectionReports').innerHTML = '<div class="empty" style="padding:40px">Loading reports…</div>';
  try {
    ...
    document.getElementById('sectionReports').innerHTML = '<div class="empty" style="padding:40px">Failed to load reports</div>';
...
export async function genReport() {
  ...
  try {
    ...
    let ords   = (await api.getOrders()).filter(o => new Date(o.createdAt) >= cut);
    ...
    if (out) out.innerHTML = '<div class="empty" style="padding:20px">Failed to generate report</div>';
    window.showToast('Report error: ' + err.message, 'toast-red');
  }
}
