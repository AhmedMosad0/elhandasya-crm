...
export async function renderProducts() {
  ...
  if (!sec) return;
  sec.innerHTML = '<div class="empty" style="padding:40px">Loading products…</div>';
  try {
    _products = await api.getProducts();
    _draw();
  } catch (err) {
    sec.innerHTML = '<div class="empty" style="padding:40px">Failed to load products</div>';
...
function _draw() {
  ...
  <div class="ph">
    <div class="ph-info">
      ...
      <p>${_products.length} products across 6 categories</p>
    </div>
  ...
  <div class="fbar" style="flex-wrap:wrap;gap:7px;align-items:center">
    <input class="fi" placeholder="Search products…" style="max-width:220px;margin:0" oninput="_prodSearch(this.value)" value="${_pQuery}">
    <button class="ftab${!_pCat ? ' active' : ''}" data-cat="" onclick="_prodCat(this.dataset.cat)">All (${_products.length})</button>
    ${CATS.map(c => {
  ...
  <div class="cards-grid" style="margin-top:16px">
    ${list.length
      ? list.map(_card).join('')
      : '<div class="empty" style="grid-column:1/-1"><div class="empty-icon">◫</div><h4>No products found</h4><p>Adjust the search or filter</p></div>'}
  </div>`;
}
...
function _card(p) {
  const img = p.imageUrl
    ...
    : `<div style="height:72px;background:var(--border-l);border-radius:8px;display:flex;align-items:center;justify-content:center;margin-bottom:10px;font-size:26px;color:var(--mute)">◫</div>`;
  ...
  return `<div class="rcard">
    ...
    <div class="rcard-actions">
      <button class="btn btn-ghost btn-xs" style="flex:1" data-pid="${p.id}" onclick="openProductForm(this.dataset.pid)">✏ Edit</button>
      <button class="btn btn-danger btn-xs" style="flex:1" data-pid="${p.id}" onclick="_deleteProd(this.dataset.pid)">✕ Delete</button>
    </div>
  </div>`;
}
...
export function openProductForm(id) {
  ...
  const imgBlock = p?.imageUrl
    ...
    : `<div style="height:72px;background:var(--border-l);border-radius:8px;display:flex;align-items:center;justify-content:center;margin-bottom:8px;font-size:22px;color:var(--mute)">◫ No image</div>`;

  openModal(`
  <div class="mh">
    <div class="mh-left"><h3>${p ? 'Edit Product' : 'New Product'}</h3></div>
    <button class="mx" onclick="closeModal()">✕</button>
  </div>
  <div class="mc">
    <div class="fr2">
      <div class="fg"><label class="fl req">Name (English)</label><input class="fi" id="pf_nameEn" value="${p?.nameEn || ''}"></div>
      <div class="fg"><label class="fl req">Name (Arabic)</label><input class="fi" id="pf_nameAr" dir="rtl" value="${p?.nameAr || ''}"></div>
    </div>
    <div class="fr2">
      <div class="fg"><label class="fl">Description (EN)</label><textarea class="fi" id="pf_descEn" rows="2" style="resize:none">${p?.descEn || ''}</textarea></div>
      <div class="fg"><label class="fl">Description (AR)</label><textarea class="fi" id="pf_descAr" dir="rtl" rows="2" style="resize:none">${p?.descAr || ''}</textarea></div>
    </div>
    <div class="fg"><label class="fl req">Category</label>
      <select class="fi" id="pf_cat"><option value="">— Select —</option>${catOpts}</select>
    </div>
    <div class="fsec">Variant Prices (EGP)</div>
    ...
    <div class="fg">
      ...
      <div style="font-size:11.5px;color:var(--mute);margin-top:4px">JPG, PNG or WebP · Uploaded to Cloudinary</div>
    </div>
  </div>
  <div class="mf">
    <button class="btn btn-ghost" onclick="closeModal()">Cancel</button>
    <button class="btn btn-primary" onclick="saveProductForm()">Save Product</button>
  </div>`, 'modal-lg');
}
...
window._deleteProd = async function (id) {
  ...
  if (!confirm(`Delete "${p?.nameEn || id}"? This cannot be undone.`)) return;
  try {
